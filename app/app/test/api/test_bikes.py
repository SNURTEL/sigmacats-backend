...
def test_read_bikes_different_rider(rider2_client, rider1, db, bike_road, bike_fixie):
...
def test_read_bike_different_rider(rider2_client, rider1, db, bike_road, bike_fixie):
    response = rider2_client.get(f"/api/rider/bike/{bike_road.id}")
    assert response.status_code == 200
    assert response.json() == jsonable_encoder(bike_road)


def test_read_bike_404_non_existent(rider1_client, rider1, db, bike_road, bike_fixie):
...
def test_update_bike_different_rider_403(rider2_client, rider1, db, bike_road):
    json = {
        "name": "Amazon Special",
        "type": BikeType.road.value,
        "brand": "Eurobike",
